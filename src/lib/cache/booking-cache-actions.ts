...
/**
 * Server Action: lấy tất cả hold đang chờ duyệt của 1 sale (created_by).
 * Dùng cho section "Hold chờ duyệt" — hiện xuyên suốt kể cả khi đổi villa.
 */
export async function fetchSaleHoldsAction(createdBy: string): Promise<Booking[]> {
  const serviceKey  = process.env.SUPABASE_SERVICE_ROLE_KEY;
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
  if (!serviceKey || !supabaseUrl) return [];

  const res = await fetch(
    `${supabaseUrl}/rest/v1/bookings?created_by=eq.${createdBy}&status=eq.hold&select=*&order=hold_expires_at.asc`,
    {
      headers: { apikey: serviceKey, Authorization: `Bearer ${serviceKey}` },
      cache: 'no-store',
    }
  ...
  return rows
    .filter((r: any) => r.hold_expires_at && new Date(r.hold_expires_at) > now)
    .map((r: any) => ({
      id:              r.id,
      villaId:         r.villa_id,
      ownerId:         r.owner_id,
